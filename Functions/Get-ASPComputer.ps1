Function Get-ASPComputer {
    <#
    .SYNOPSIS
    Pulls down ADUC information from target ASP, using the PoSH Active Directory Module
    .DESCRIPTION
    Asks for credentials based on the target ASP. Then pulls all of the ASP information down for ALL computers in the ASP domain.
    Due to a bug in Get-ADComputer which causes PoSH to slow down when results are saved in an object or an array, I've had the
    ...
    .EXAMPLE
    Get-ASPComputer Username 99 x.x.55.555
    .PARAMETER Account
    ...
    Get-ADComputer -Filter * -Server $DC -Credential $GetCred -Properties * | Export-Csv "C:\Windows\Temp\Get-ASPComputer-$ASP-Temp.csv" -Force
    Import-Csv "C:\Windows\Temp\Get-ASPComputer-$ASP-Temp.csv"
    Remove-Item "C:\Windows\Temp\Get-ASPComputer-$ASP-Temp.csv"
}