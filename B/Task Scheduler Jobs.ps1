...
$Computers = Get-Content "C:\Users\\Desktop\Comps.txt"

# Save path for the results
$Date = Get-Date -UFormat "%d-%b-%g %H%M"
$Path = "C:\Users\\Documents\Task Schedule $Date.csv"
...
$ScriptBlock = {
    ...
    {    
        If (Test-Connection $args[0] -Quiet -Count 1 -BufferSize 16 -Ea 0)
        {
            ...
            $Task = schtasks.exe /CREATE /TN "Air" /S $args[0] /SC ONLOGON /RU "SYSTEM" /TR "powershell.exe -WindowStyle Hidden -noprofile -File '\\xlin\Uninstall App.ps1'" /F
            Sleep -s 1
            $Run = schtasks.exe /RUN /TN "Air" /S $args[0] 
            Sleep -s 1
            $Delete = schtasks.exe /DELETE /TN "Air" /S $args[0] /F
            $Success = "Successful"
        }
        Else
        {
