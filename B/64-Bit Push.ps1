...
$ScriptBlock = {
    If (Test-Connection $args[0] -Quiet -Count 1 -BufferSize 16 -Ea 0)
    {
        ...
        Try
        {
            #$Task = schtasks.exe /CREATE /TN "64Bit Upgrade" /S $args[0] /SC ONCE /ST 16:00 /RU "INTERACTIVE" /TR "powershell.exe -noprofile -File '\\64Bit_Install.ps1'" /F
