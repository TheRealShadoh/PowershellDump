...
$ScriptBlock = {
    ...
    {    
        If (Test-Connection $args[0] -Quiet -Count 1 -BufferSize 16 -Ea 0)
        {
            ...
            $Task = schtasks.exe /CREATE /TN "Reboot" /S $args[0] /SC ONCE /ST 18:00 /RU "INTERACTIVE" /TR "powershell.exe -ExecutionPolicy Unrestricted -WindowStyle Hidden -noprofile -File '\\xlmin\Progess Bar Reboot.ps1'" /F
            Sleep -s 1
