...
$Users = Get-ADUser -Filter * -SearchBase "OU= ,OU=,OU=,DC=,DC=,DC=,DC=" -Properties DisplayName, canonicalName, Organization, Description, SmartcardLogonRequired | Where {$_.SmartcardLogonRequired -eq $false}
$SUsers = Get-ADUser -Filter * -SearchBase "OU= ,OU=,OU=,DC=,DC=,DC=,DC=" -Properties DisplayName, canonicalName, Organization, Description, SmartcardLogonRequired
$AUsers = Get-ADUser -Filter * -SearchBase "OU= ,OU=,OU=,DC=,DC=,DC=,DC=" -Properties DisplayName, canonicalName, Organization, Description, SmartcardLogonRequired
